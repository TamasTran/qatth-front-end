...
export default function CVScanner() {
  ...
  return (
    <div className="grid md:grid-cols-2 gap-6">
      <div className="card">
        <h2 className="font-semibold mb-2">Tải lên hoặc dán CV của bạn</h2>
        <p className="text-sm text-slate-400 mb-4">Hỗ trợ PDF/TXT. Phân tích diễn ra trên trình duyệt — dữ liệu của bạn an toàn.</p>
        ...
        {loading && <p className="text-sm text-slate-300 mt-2">Đang đọc tệp...</p>}
        {error && <p className="text-sm text-red-300 mt-2">{error}</p>}
      </div>
      <div className="space-y-6">
        <div className="card">
          <h3 className="font-semibold mb-2">Kỹ năng phát hiện</h3>
          <div className="flex flex-wrap gap-2">
            {skills.length ? skills.map(s=>(<span key={s} className="px-2 py-1 rounded-full bg-white/10 text-sm">{s}</span>)) : <p className="text-slate-400 text-sm">Chưa có dữ liệu.</p>}
          </div>
        </div>
        <div className="card">
          <h3 className="font-semibold mb-3">Gợi ý nghề nghiệp phù hợp</h3>
          <div className="space-y-3">
            {roles.slice(0,5).map(r=>(
              <div key={r.id} className="flex items-center justify-between">
                <div>
                  <div className="font-medium">{r.title}</div>
                  <div className="text-xs text-slate-400">Độ phù hợp: {(r.score*100).toFixed(0)}% — Trùng {r.overlap} kỹ năng</div>
                </div>
                <Link to="/jobs" className="btn btn-primary">Xem việc phù hợp</Link>
              </div>
            ))}
            {!roles.length && <p className="text-slate-400 text-sm">Hãy tải lên hoặc dán CV để xem gợi ý.</p>}
          </div>
        </div>
        <div className="card">
          <h3 className="font-semibold mb-2">Tiếp theo</h3>
