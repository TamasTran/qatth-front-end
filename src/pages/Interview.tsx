    ...
    <div className="grid md:grid-cols-3 gap-6">
      <div className="card md:col-span-2">
        <div className="flex items-center justify-between mb-2">
          <h2 className="font-semibold">Phỏng vấn mô phỏng bằng giọng nói</h2>
          <select value={role} onChange={e=>setRole(e.target.value)} className="bg-slate-900 border border-white/10 rounded-xl p-2">
        ...
        <div className="rounded-xl border border-white/10 p-4 bg-slate-900/40">
          <div className="text-sm text-slate-300">Câu hỏi {idx+1}/{questions.length}</div>
          <div className="text-lg mt-2">{questions[idx].q}</div>
          ...
          <div className="mt-4">
            <div className="text-sm text-slate-400">Bản ghi câu trả lời:</div>
            <div className="min-h-[80px] rounded-xl bg-slate-950/60 border border-white/10 p-3">{transcript || '—'}</div>
            <div className="mt-2 text-sm">Điểm từ khoá: <span className="text-brand-400">{scores[idx] ?? 0}</span>/100</div>
          </div>
        </div>
      </div>
      <div className="card">
        <h3 className="font-semibold mb-2">Mẹo trả lời tốt</h3>
        ...
        <div className="mt-4">
          <h4 className="font-medium mb-1">Kết quả phiên</h4>
          <div className="text-sm text-slate-300">Điểm trung bình: {Math.round( (scores.reduce((a,b)=>a+(b||0),0) / Math.max(1,scores.length)) || 0 )}/100</div>
        </div>
      </div>
    </div>
  )
